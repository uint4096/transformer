export const store = () => {
    ...
    return {
        setMetadata: <T extends { new(): InstanceType<T> }>(target: T, key: any, func: any) => {
            if (store.has(target.constructor)) {
                ...
                if (keyStore.has(key)) {
                    ...
                    const transformerStore = (new Map()).set(key, func);
                    keyStore.set(key, transformerStore);
                }
            } else {
        ...
        getTransformer: <T extends { new(): InstanceType<T> }>(target: T, key: keyof T) => {
